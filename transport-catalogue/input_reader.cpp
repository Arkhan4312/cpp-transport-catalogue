...
CommandDescription ParseCommandDescription(std::string_view line) {
    ...
    return {std::string(line.substr(0, space_pos)), std::string(line.substr(not_space, colon_pos - not_space)),
            std::string(line.substr(colon_pos + 1))};
}

ParsedDistance ParseDistance(std::string_view part) {
...
void InputReader::AddBuses(TransportCatalogue& catalogue) const {
    for (const auto& cmd : commands_) {
        if (cmd.command == "Bus") {
            auto route_stops_view = detail::ParseRoute(cmd.description);
            bool is_ring = (cmd.description.find('>') != std::string::npos);
            catalogue.AddBus(cmd.id, route_stops_view, is_ring);
        }
    }
}
